...
我发现其声明式的语言风格和Google最新推荐的标准Jetpack Compose十分类似，是视图和控制逻辑结合的响应式编程，其页面重组，数据传递和状态迁移相较传统的view模式有显著的提升，特别是其页面重组可以由状态树来最小化重组规模从而减小开销。刚好我的Android应用Wordle也是采用声明式的Jetpack Compose开发，所以我发现其开发模式很容易上手。阅读示例项目基本不需要查阅文档。

不过DevEco Studio与Android Studio不同，使用模拟器竟然需要申请资格，资格审核用了几天时间。

虽然课题研究内容不在应用层面上，但我希望能从应用层了解应用是如何与操作系统打交道的，毕竟Open Harmony系统设计吸取了过去操作系统的经验和教训也采用了新的设计理念，例如微内核架构，分布式架构等，其应用使用的接口和我过去开发的Android应用有我所不了解的区别，了解了应用层如何开发设计才能更好的在操作系统层面做出有利于应用使用的改进。
